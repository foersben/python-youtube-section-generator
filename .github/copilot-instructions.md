...
- Source code → `src/**/*.py`
- Never create summary/report files in project root - use `docs/` instead
...
def validate_sections(
    ...
    """Validate and fix section data."""
    
    # Sort by start time
    sections = sorted(sections, key=lambda s: s.get("start", 0))
    
    # Validate bounds
    max_time = max(t["start"] + t.get("duration", 0) for t in transcript)
    for section in sections:
        if not isinstance(section.get("start"), (int, float)):
            section["start"] = 0.0
        section["start"] = max(0.0, min(float(section["start"]), max_time))
...
- ❌ **Don't create files outside project structure** (no /tmp/, no /, no project root)
- ❌ **Don't create test files anywhere except `tests/` directory**
- ❌ **Don't create documentation anywhere except `docs/` directory**
- ❌ **Don't create summary/report files in project root** - use `docs/` instead
- ❌ Don't hardcode API keys or sensitive data in source files
- ❌ Don't use `List`, `Dict`, `Tuple` from typing (use built-ins for Python 3.11+)
- ❌ Don't omit module docstrings
- ❌ Don't create functions longer than ~50 lines without good reason
- ❌ Don't ignore logging - always log important events and errors
- ❌ Don't catch generic `Exception` without re-raising or logging
- ❌ Don't assume API responses are valid - always validate
- ❌ Don't use bare `except:` (always catch specific exceptions)
- ❌ Don't forget to handle edge cases (empty data, None values, network failures)
- ❌ Don't use `print()` for debugging - use `logger.debug()` instead
- ❌ Don't modify global state without good reason
- ❌ Don't return different types from the same function
- ❌ Don't use synchronous blocking calls in async contexts

## Dependencies
- Python 3.11+ (supports built-in generics, improved type hints)
- **youtube-transcript-api**: YouTube transcript extraction
- **google-genai**: Google Gemini AI client (optional, for cloud API)
- **transformers**: HuggingFace transformers for local LLMs
...
## Development Tools
- **pytest**: Testing framework (recommended to add)
- **mypy**: Static type checker (recommended to add)
- **black**: Code formatter (recommended to add)
- **ruff**: Fast Python linter (recommended to add)
- **langdetect**: Language detection for transcripts
...
### YouTube Video ID Handling
- Always validate video IDs (11 characters)
- Support both full URLs and bare IDs
- Use regex for extraction: `r"(?:v=|/)([0-9A-Za-z_-]{11})"`
- Log the extracted video ID for debugging

### AI Response Handling
- Always validate AI-generated JSON before using
...
- Cache AI responses when appropriate to save costs

### Translation Support
- Detect language before translating
- Only translate if target language differs from source
- Handle translation API failures gracefully
- Log detected languages for monitoring

### Timestamp Formatting
- Store timestamps as floats (seconds)
- Format for YouTube as MM:SS
- Validate timestamps are within video duration
- Sort sections by timestamp before output

### Configuration Management
- Use `.env` file for local development
- Support frozen apps (PyInstaller) by checking `sys.frozen`
- Provide sensible defaults for all configuration
- Document required environment variables in README

### Local LLM Configuration
- Use `USE_LOCAL_LLM=true` to switch from Gemini API to local model
- Default model: `meta-llama/Meta-Llama-3-8B-Instruct`
- Enable 4-bit quantization with `LOCAL_MODEL_4BIT=true` to reduce memory usage
- Local models require ~5-8GB RAM with quantization, ~16GB without
- First model load downloads weights from HuggingFace (~15GB)
- Requires HuggingFace account and accepted license for Llama models
- CUDA GPU recommended but CPU inference is supported (slower)
