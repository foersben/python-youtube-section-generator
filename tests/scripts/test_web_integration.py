...
if not model_path.exists():
    pytest.skip(f"Local model not present at {model_path}; skipping heavy integration test", allow_module_level=True)

try:
    from src.core.services.section_generation import SectionGenerationService
except Exception:
    pytest.skip("SectionGenerationService not available; skipping integration test", allow_module_level=True)
